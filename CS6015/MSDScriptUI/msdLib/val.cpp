...
bool FunVal::equals(PTR(Val) other) {
    ...
    return f != nullptr &&
           ...
           body->equals(f->body) &&
           (env == f->env || (env != nullptr && f->env != nullptr && env->equals(f->env)));
}
...
PTR(Val) FunVal::call(PTR(Val) actual_arg_val) {
    //Create a new environment with the argument bound to its value
    PTR(Env) new_env = NEW(ExtendedEnv)(formal_arg, actual_arg_val, env);
