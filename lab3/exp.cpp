...
int Num::interp() {
    return val; // number itself
}

bool Num::has_variable() {
    return false; // number never contains a variable
}
...
int Add::interp() {
    return lhs->interp() + rhs->interp(); // addition is the sum of its subexpressions
}
...
int Mult::interp() {
    return lhs->interp() * rhs->interp(); //multiplication is the product of its subexpressions
}
...
int VarExpr::interp() {
    throw std::runtime_error("no value for variable"); //variable has no value
}

bool VarExpr::has_variable() {
    return true; //variable always contains a variable
}

Expr* VarExpr::subst(const std::string& variable, Expr* replacement) {
    if (name == variable) {
        return replacement; // Replace the variable
    } else {
        return new VarExpr(name); // return a new VarExpr with the same name
    }
}
